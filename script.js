...
function start() {
    ...
    const DOM = {
      ...
      addTransaction(transaction, index) {
        ...
        image.addEventListener("click", () => {
          ...
          window.NotificateUser(`O Vôo ${description} foi cancelado com sucesso, nos relate o motivo`);
          if (window.userAcceptedVoice) {
            window.notificateUserByVoice(
              `O Vôo ${description} foi cancelado com sucesso, nos relate o motivo`
            );
          }
        });
    ...
    formElement.addEventListener("submit", (event) => {
      ...
      try {
        ...
        window.NotificateUser(
          `O Vôo ${transaction.description} foi incluido na sua agenda`
        );
        if (window.userAcceptedVoice) {
          window.notificateUserByVoice(
            `O Vôo ${transaction.description} foi incluido na sua agenda`
  ...
  function DarkMode() {
    ...
    function changeText(hasDarkMode,showNotification=true) {
      ...
      p.textContent = hasDarkMode ? "Vôos" : "Vagas ";
      darkModeCheckbox.checked = p.textContent === "Vôos";
      if(!showNotification) return;
      window.NotificateUser(
        `Watson Buscar vôos conforme meu saldo total de milhas`
      );
      if (window.userAcceptedVoice) {
        window.notificateUserByVoice(
          `Watson Buscar vôos conforme meu saldo total de milhas}`
  ...
  function DownloadJSON() {
    ...
    saveJsonButton.addEventListener("click", () => {
      ...
      downloadModal.classList.remove("active");
      window.NotificateUser(`Watson Buscar vôos conforme meu saldo total de milhas ${filename}`);
      if (window.userAcceptedVoice) {
        window.notificateUserByVoice(`Watson Buscar vôos conforme meu saldo total de milhas`);
      }
    });
  }
